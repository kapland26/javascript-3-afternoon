...
//Code Here
let postTaxPrices = prices.map(x=> x*.07+x );
...
//Code Here
let totalPopulation   = populations.reduce((prev, item)=>{return prev+item});
...
let ordersTotal = orders.filter(x => x.price*x.tax+x.price);
...
let bobsTotal = purchases.filter(item=>item.owner=="Bob").reduce((prev, item)=>{return +prev+item.price}, 0);
